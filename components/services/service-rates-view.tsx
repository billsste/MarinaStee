...
type Tab = "slip_pricing" | "other_rates" | "fees";

// ─────────────────────────────────────────────────────────────────────
// Fee constants (mirror what used to live in fees-manager.tsx).
// ─────────────────────────────────────────────────────────────────────

const FEE_CADENCE_ORDER: FeeRecurrence[] = ["one_time", "monthly", "annual"];

const FEE_CADENCE_META: Record<
  FeeRecurrence,
...
const FEE_CADENCE_OPTIONS = FEE_CADENCE_ORDER.map((c) => ({
  value: c,
  label: FEE_CADENCE_META[c].label,
}));

const FEE_APPLIES_LABEL: Record<FeeAppliesTo, string> = {
...
const FEE_ENTITY_ORDER: FeeEntity[] = [
  "reservation",
  "contract",
  "club_subscription",
  "rental_boat",
...
const FEE_ENTITY_LABEL: Record<FeeEntity, string> = {
...
const FEE_ENTITY_OPTIONS = FEE_ENTITY_ORDER.map((e) => ({
  value: e,
  label: FEE_ENTITY_LABEL[e],
}));

// Grid for the Fees table:
// NAME · CADENCE · APPLIES TO (entities) · SURFACES · QB LINE · AMOUNT · USAGE · trash slot
const FEE_COLS =
 ...
 */
function effectiveFeeCadence(f: AdditionalFee): FeeRecurrence {
...
// ─────────────────────────────────────────────────────────────────────
// Tab 3 — Fees (AdditionalFee rows).
// Folds the prior /services/fees content into this unified surface.
// Sectioning REMOVED — the prior fees-manager rendered per-cadence
// subheader rows ("ONE-TIME (4)", "MONTHLY (2)") between groups, which
// violated CLAUDE.md rule §"List-page UX consistency" #1 ("NEVER
// section the list by category"). Cadence is now expressed entirely
// through the filter chip's live counts.
// ─────────────────────────────────────────────────────────────────────
...
function FeesTab({
  ...
  const counts = React.useMemo(() => {
    const out: Record<FeeRecurrence, number> = {
      one_time: 0,
      monthly: 0,
      annual: 0,
    };
    for (const f of fees) out[effectiveFeeCadence(f)]++;
    return out;
  }, [fees]);
      ...
      .filter((f) => {
        ...
        if (entityFilter !== "all") {
          ...
          if (
            entities &&
            entities.length > 0 &&
            !entities.includes(entityFilter as never)
          ) {
            return false;
          }
        }
        if (query.trim().length > 0) {
          const q = query.trim().toLowerCase();
          const hay =
            `${f.name} ${f.description ?? ""} ${f.accounting_line_item ?? ""}`.toLowerCase();
          if (!hay.includes(q)) return false;
        }
        return true;
      })
      .sort((a, b) => {
        const ca = FEE_CADENCE_ORDER.indexOf(effectiveFeeCadence(a));
        const cb = FEE_CADENCE_ORDER.indexOf(effectiveFeeCadence(b));
  ...
  function handleSave(values: AdditionalFee) {
    // applies_to_entities semantics: undefined = "applies to all entities".
    ...
    const rawEntities = values.applies_to_entities;
    ...
    const final: AdditionalFee = {
      ...
      linked_template_id:
        values.linked_template_id ?? editing?.linked_template_id,
      cadence,
      recurrence: cadence,
  ...
  const FEE_FIELDS: FieldSpec<AdditionalFee>[] = React.useMemo(
    () => [
      {
        key: "name",
        label: "Fee name",
  ...
  return (
    <>
      ...
      <div className="mt-4 overflow-hidden rounded-[12px] border border-hairline bg-surface-1">
        ...
        {sortedFees.length === 0 ? (
          <div className="px-4 py-10 text-center text-[13px] text-fg-subtle">
            No fees match these filters. Click{" "}
            <span className="font-medium text-fg">New fee</span> to add one.
          </div>
        ) : (
          <ul className="divide-y divide-hairline">
            {sortedFees.map((f) => {
              const cadence = effectiveFeeCadence(f);
              const meta = FEE_CADENCE_META[cadence];
      ...
      <div className="text-[11px] text-fg-tertiary">
        {sortedFees.length} of {fees.length} fee{fees.length === 1 ? "" : "s"}
      </div>
      ...
      <RecordEditDialog<AdditionalFee>
        ...
        onOpenChange={setDialogOpen}
        title={
          editing ? `Configure fee — ${editing.name}` : "New additional fee"
        }
