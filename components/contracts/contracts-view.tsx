...
import { useContracts } from "@/lib/client-store";
import type { Contract, ContractStatus, ContractTemplate } from "@/lib/types";

/*
 * Templates library + flat contracts table. Was the entire contracts page;
 * now a secondary tab (the renewal pipeline is the primary).
 */
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3 lg:grid-cols-4">
        ...
        <KpiCard
          label="Templates"
          value={`${CONTRACT_TEMPLATES.length}`}
          sub={`${CONTRACT_TEMPLATES.filter((t) => t.auto_renew).length} auto-renew`}
        />
      </div>
